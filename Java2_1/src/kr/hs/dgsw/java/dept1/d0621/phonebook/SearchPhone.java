...
public class SearchPhone {
	...
	static void searchphone(Scanner sc, List<NumberInfo> number, String str) { // 전화번호로 검색하기
		List<NumberInfo> temp = new ArrayList<>(); // 임의의 temp라는 리스트를 만든다.
		for(int i=0; i<number.size(); i++) { // number의 size만큼 for문을 돈다.
			NumberInfo searchnumber = (NumberInfo) number.get(i); // number라는 list안에 i번쨰 값을 들고온더.
			if(searchnumber.getPhone().contains(str)) { // 사용자가 입력한 이름이 i번째 값에 포함되는지 확인
